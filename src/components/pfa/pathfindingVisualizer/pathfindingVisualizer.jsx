...
// importMaze Algorithms
import { randomMaze } from "../mazeAlgorithms/randomMaze";

const initialNum = getInitialNum(window.innerWidth, window.innerWidth);
const initialNumRows = initialNum[0];
const initialNumColumns = initialNum[1];
console.log(initialNum, initialNumColumns )
...
class PathfindingVisualizer extends Component {
  ...
  componentDidMount() {
    // check if the window is resized and update grids if necessary

    window.addEventListener("resize", this.updateDimensions);
    const grid = getInitialGrid(this.state.numRows, this.state.numColumns);
    this.setState({ grid });
  }
  // handle mouse events click garda, drag garda , unclick garda k k hunxa vanera change garne
